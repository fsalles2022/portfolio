<script setup></script>
<template>
    <footer class="p-4 bg-dark-primary md:p-8 lg:p-10 dark:bg-gray-800 hover:bg-teal-950 ">
        <div class="mx-auto max-w-screen-xl text-center text-amber-300">
            <a
                href="/"
                class="flex justify-center items-center text-2xl font-semibold text-gray-900 dark:text-white"
            >
                <img
                    src="https://planbwebapp.com.br/images/logo2.webp"
                    class="mr-3 h-6 sm:h-9 "
                    alt="Planbwebapp Logo"
                />
                <p class="text-lime-400">Planbwebapp™</p>
            </a>
            <p class="my-6 text-lime-400 dark:text-gray-400">
                Desenvolvimento de sistemas para a web - Entre em contato 11 91834-3312 ou 11
                95555-4621
            </p>
            <ul
                class="flex flex-wrap justify-center items-center mb-6  text-lime-400 dark:text-white hover"
            >
                <li>
                    <a href="#about" class="mr-4 hover:underline md:mr-6"
                        >Sobre</a
                    >
                </li>
                <li>
                    <a href="#portfolio" class="mr-4 hover:underline md:mr-6"
                        >Portfolio</a
                    >
                </li>
                <li>
                    <a href="#services" class="mr-4 hover:underline md:mr-6"
                        >Serviços</a
                    >
                </li>
                <li>
                    <a href="#contact" class="mr-4 hover:underline md:mr-6"
                        >Contato</a
                    >
                </li>
            </ul>
            <span
                class="text-sm text-lime-400 sm:text-center  dark:text-gray-400"
                >© 2021-2023
                <a href="/" class="hover:underline">Planbwebapp™ </a> -  Direitos
                Reservados.</span
            >
       
        </div>
    </footer>
</template>
